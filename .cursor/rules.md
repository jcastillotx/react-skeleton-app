...
1. **Outcome**: What outcome should the user see when this is done?
2. **Stack**: Which stack is required: Laravel app, Next.js app, or WordPress plugin?
3. **Deployment**: Where is the deployment target: WHM AWS LAMP server, Vercel, or existing React server?
4. **Scope**: What is in scope vs. explicitly out of scope?
5. **Constraints**: Are there constraints (deadlines, tech choices, must-not-change areas)?
6. **Acceptance**: What are the acceptance criteria and how will we verify success?
7. **Testing**: What tests or checks are expected?
